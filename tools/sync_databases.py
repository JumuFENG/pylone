...
import app.stock.models as stock_models
import app.selectors.models as selectors_models
...
class DatabaseSyncManager:
    ...
    async def initialize(self):
        ...
        self.mysql_base = declarative_base()
    ...
    async def get_model_classes(self, engine=None) -> List[type]:
        """Return mapped ORM model classes whose tables exist in the given engine.

        If `engine` is None the manager will try `self.mysql_engine` then `self.sqlite_engine`.
        """
        engine = engine or self.mysql_engine or self.sqlite_engine

        # collect ORM classes from the three modules
        known: List[type] = []
        for mod in (users_models, stock_models, selectors_models):
        ...
        # If no engine available (not initialized), return the known list
        if engine is None:
            return known
        ...
        return [m for m in known if getattr(m, "__tablename__", None) in table_names]
    ...
    async def sync_table(self, model_class, direction: str = 'mysql_to_sqlite',
        ...
        if not await self.table_exists(target_engine, table_name):
            print(f"  目标表 {table_name} 不存在，正在创建")
            async with target_engine.begin() as conn:
                await conn.run_sync(lambda sync_conn: model_class.__table__.create(sync_conn, checkfirst=True))
    ...
    async def sync_all_tables(self, direction: str = 'mysql_to_sqlite',
        ...
        """
        results = {}
        model_classes = await self.get_model_classes(self.mysql_engine if direction == 'mysql_to_sqlite' else self.sqlite_engine)
...
async def main():
    ...
    try:
        ...
        if args.table:
            # Sync specific table
            model_classes = await sync_manager.get_model_classes(sync_manager.mysql_engine if args.direction == 'mysql_to_sqlite' else sync_manager.sqlite_engine)
            target_model = None
            for model_class in model_classes:
